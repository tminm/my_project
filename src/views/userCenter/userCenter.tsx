...
import Assess from "views/assess/assess";

const UserCenter = memo(() => {
  // 获取 Redux store 中的 userInfo 和 orderInfo
  const { userInfo, orderInfo } = useSelector((state: any) => ({
    userInfo: state.home.userInfo, // 根据实际 Redux 状态修改路径
    orderInfo: state.detail.orderInfo,
  }));

  const confirmInfo = useSelector(
    (state: { detail: { confirmInfo: any } }) => state.detail.confirmInfo
  );
